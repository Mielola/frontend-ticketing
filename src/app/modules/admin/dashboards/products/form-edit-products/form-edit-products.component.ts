...
@Component({
  selector: 'app-form-edit-products',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    FormsModule,
  ...
  templateUrl: './form-edit-products.component.html',
  styleUrl: './form-edit-products.component.scss'
})
export class FormEditProductsComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<FormEditProductsComponent>,
  ...
  async fetchProductsById() {
    try {
      const response = await this._apiService.get(`api/V1/products/${this.id}`)
  ...
  async onSubmit() {
    ...
    try {
      this.isLoading = true
      const { data, status } = await this._apiService.post(`api/V1/products/${this.id}`, {
